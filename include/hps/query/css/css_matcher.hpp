...
class CSSMatcher {
  public:
     ...
     * @return 匹配的元素列表（去重）
     */
    static std::vector<const Element*> find_all(const Element& element, const SelectorList& selector_list);
     ...
     * @return 第一个匹配的元素，如果没有找到返回nullptr
     */
    static const Element* find_first(const Element& element, const SelectorList& selector_list);
  ...
  private:
     ...
     * @param results 结果容器
     */
    static void traverse_and_match(const Element& element, const CSSSelector& selector, std::vector<const Element*>& results);
     ...
     * @param results 结果容器
     */
    static void traverse_and_match(const Element& element, const SelectorList& selector_list, std::vector<const Element*>& results);
};
