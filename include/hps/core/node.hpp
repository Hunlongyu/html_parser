...
enum class NodeType {
    ...
    Attribute,              ///< 属性节点 id="1"
    Text,                   ///< 文本节点 <p>Hello</p>
    CDataSection,           ///< CDATA节点 <![CDATA[hello world]]>
    EntityReference,        ///< 实体引用节点 &lt; 【已弃用】
    Entity,                 ///< 实体节点 <!DOCTYPE html> 【已弃用】
    ProcessingInstruction,  ///< 指令节点 <?xml version="1.0"?>
    Comment,                ///<!-- 注释 -->
    Document,               ///< 文档节点 整个文档的根节点
    DocumentType,           ///< 文档类型节点 <!DOCTYPE html>
    DocumentFragment,       ///< 文档片段节点
    Notation,               ///< 符号节点 <!ENTITY ...> 【已弃用】
    LastEntry,              ///< 最后一个节点
};
...
class Node : public NonCopyable {
  public:
    ...
    /// @brief 获取节点内容
    /// @return 节点内容
    virtual std::string_view node_value() const = 0;

    /// @brief 递归获取节点所有文本内容
    /// @return 节点文本内容
    virtual std::string_view text_content() const = 0;
    ...
    /// @brief 获取第一个子节点
    /// @return 第一个子节点
    const Node* first_child() const noexcept;

    /// @brief 获取最后一个子节点
    /// @return 最后一个子节点
    const Node* last_child() const noexcept;
    ...
    /// @param index  索引
    /// @return 子节点
    const Node* child_at(size_t index) const noexcept;
